...
public class StorageSynchronized<T> {
	private String name;
	...
	public StorageSynchronized(int capacity, String name) {
	...
	public T get() throws InterruptedException {
		...
		try {
			...
			// now we have some place for adding new values (size decreased by
			// 1), wake up put()
			synchronized (isFull) {
				isFull.notify();
			}
	...
	public void put(T value) throws InterruptedException {
		...
		synchronized (this) {
			if (size == 0) {
				// head == tail == null element , assign new value to head
				// element
				array[head] = value;
			} else {
		...
		// now we have some objects which can be retrieved by get (size
		// increased by 1), wake up get()
		synchronized (isEmpty) {
			isEmpty.notify();
		}
	}
